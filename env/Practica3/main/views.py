...
def saldo(request):
	...
	if cuenta_obj.cantidad is None:
		cuenta_obj.cantidad = 0.0
...
def solicitar_credito(request):
	...
	if request.method == 'POST':
		#DEBITO_CREDITO
		tipo = 'Crédito'
		no_cuenta = request.session['no_cuenta']
		...
		"""
		context = {
			'mensaje' : "Se ha solicitado el crédito correctamente."
		}
		transaccion_obj = Transaccion(no_cuenta=cuenta_obj, monto=monto, descripcion=descripcion, estado='e', cod_tipo=tipo_obj)
		transaccion_obj.save()
		return render(request, 'main/solicitar_credito.html', context)
	return render(request, 'main/solicitar_credito.html', {})
	...
	if request.method == 'POST':
		...
		if cuenta_obj is None:
			...
			if float(monto) > cuenta_obj.cantidad:
				...
				transaccion_obj = Transaccion(no_cuenta=cuenta_obj, monto=monto, descripcion=descripcion, estado = 'v', cod_tipo=tipo_obj)
				transaccion_obj.save()
		return render(request, 'main/debito.html', context)
	return render(request, 'main/debito.html', context)

def solicitud_credito(request):
	if 'id' in request.GET:
		parametro = request.GET['id']
		cods = parametro.split('/')
		for cod in cods:
			if cod:
				transaccion_obj = Transaccion.objects.filter(cod=cod).first()
				no_cuenta = transaccion_obj.no_cuenta.no_cuenta
				...
				cuenta_obj.cantidad = cuenta_obj.cantidad+float(monto)
				cuenta_obj.save()
				transaccion_obj.estado = 'v'
				transaccion_obj.save()
	if 'id2' in request.GET:
		parametro = request.GET['id2']
		cods = parametro.split('/')
		for cod in cods:
			if cod:
				transaccion_obj = Transaccion.objects.filter(cod=cod).first()
				no_cuenta = transaccion_obj.no_cuenta.no_cuenta
				transaccion_obj.estado = 'f'
				transaccion_obj.save()
	creditos_obj = Transaccion.objects.all()
	context = {
		'creditos' : creditos_obj
	}
	return render(request, 'main/solicitud_credito.html', context)
